Crafty.c("Dinosaur", {
  ...
  dinosaur(props) {
    ...
    this.jaw = Crafty.e("2D, WebGL, dinoJaw, TweenPromise, Delta2D").attr({
      x: this.x - 89,
      y: this.y + 52,
      z: this.z + 1
    }).origin(70, 16)
    ...
    this.tongue = Crafty.e("2D, WebGL, dinoTongue, TweenPromise, Delta2D").attr({
      x: this.x - 39,
      y: this.y + 52,
      z: this.z - 1
    }).origin(70, 16)
  ...
  execute(pose, settings) {
    switch(pose) {
      ...
      case "roar": return this.poseRoar()
      case "run": return this.run(settings)
    }
  },

  async setPose(rotations, overall, duration) {
    const elements = Object.keys(rotations).map(element =>
      this[element].tweenPromise(this.convertToProps(rotations[element]), duration)
    )
    await Promise.all([
      ...elements,
